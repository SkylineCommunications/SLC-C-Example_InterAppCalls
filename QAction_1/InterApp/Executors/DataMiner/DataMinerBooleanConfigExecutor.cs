...
namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataMinerBooleanConfigExecutor : MessageExecutor<GenericInterAppMessage<DataMinerBooleanConfigRequest>>
	{
		private DataMinerBooleanConfigResponse _response;

		public DataMinerBooleanConfigExecutor(GenericInterAppMessage<DataMinerBooleanConfigRequest> message) : base(message)
		{
			_response = new DataMinerBooleanConfigResponse
			{
				Success = false,
		...
		public override void DataSets(object dataDestination)
		{
			...
			try
			{
				protocol.SetParameter(Parameter.generic_dm_directvalidation_configbool_1005, Convert.ToInt16(Message.Data.Config));
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataMinerBooleanConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataMinerBooleanConfigResponse>(_response);
			}
