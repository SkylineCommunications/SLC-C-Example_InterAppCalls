namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataMinerDiscreetConfigExecutor : MessageExecutor<GenericInterAppMessage<DataMinerDiscreetConfigRequest>>
	{
		private DataMinerDiscreetConfigResponse _response;

		public DataMinerDiscreetConfigExecutor(GenericInterAppMessage<DataMinerDiscreetConfigRequest> message) : base(message)
		{
			_response = new DataMinerDiscreetConfigResponse
			{
				Success = false,
		...
		public override bool Validate()
		{
			...
			if (!Enum.IsDefined(typeof(DataMinerDiscreet), Message.Data.Config))
			{
				_response.Description = "The provided config value is not a valid DataMinerDiscreet value.";
				return false;
			}
		...
		public override void DataSets(object dataDestination)
		{
			...
			try
			{
				protocol.SetParameter(Parameter.generic_dm_directvalidation_configdiscreet_1004, Convert.ToInt16(Message.Data.Config));
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataMinerDiscreetConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataMinerDiscreetConfigResponse>(_response);
			}
