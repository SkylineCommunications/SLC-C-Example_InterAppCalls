namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataMinerStringConfigExecutor : MessageExecutor<GenericInterAppMessage<DataMinerStringConfigRequest>>
	{
		private DataMinerStringConfigResponse _response;

		public DataMinerStringConfigExecutor(GenericInterAppMessage<DataMinerStringConfigRequest> message) : base(message)
		{
			_response = new DataMinerStringConfigResponse
			{
				Success = false,
		...
		public override void DataSets(object dataDestination)
		{
			...
			try
			{
				protocol.SetParameter(Parameter.generic_dm_directvalidation_configstring_1002, Message.Data.Config);
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataMinerStringConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataMinerStringConfigResponse>(_response);
			}
