namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataSourceBooleanConfigExecutor : MessageExecutor<GenericInterAppMessage<DataSourceBooleanConfigRequest>>
	{
		private DataSourceBooleanConfigResponse _response;
		private bool _canExecute;

		public DataSourceBooleanConfigExecutor(GenericInterAppMessage<DataSourceBooleanConfigRequest> message) : base(message)
		{
			_response = new DataSourceBooleanConfigResponse
			{
				Success = false,
		...
		private void DataSets_Initial(SLProtocol protocol)
		{
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataSourceBooleanConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}

		private void DataSets_Next(SLProtocol protocol)
		{
			protocol.SetParameter(Parameter.generic_datasource_postvalidation_configbool_5005, Message.Data.Config);
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataSourceBooleanConfigResponse>(_response);
			}
