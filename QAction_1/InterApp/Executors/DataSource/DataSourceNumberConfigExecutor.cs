namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataSourceNumberConfigExecutor : MessageExecutor<GenericInterAppMessage<DataSourceNumberConfigRequest>>
	{
		private DataSourceNumberConfigResponse _response;
		private bool _canExecute;

		public DataSourceNumberConfigExecutor(GenericInterAppMessage<DataSourceNumberConfigRequest> message) : base(message)
		{
			_response = new DataSourceNumberConfigResponse
			{
				Success = false,
		...
		private void DataSets_Initial(SLProtocol protocol)
		{
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataSourceNumberConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}

		private void DataSets_Next(SLProtocol protocol)
		{
			protocol.SetParameter(Parameter.generic_datasource_postvalidation_confignumber_5003, Message.Data.Config);
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataSourceNumberConfigResponse>(_response);
			}
