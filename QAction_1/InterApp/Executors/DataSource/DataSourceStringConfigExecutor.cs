namespace Skyline.Protocol.InterApp.Executors.DataMiner
{
	...
	public class DataSourceStringConfigExecutor : MessageExecutor<GenericInterAppMessage<DataSourceStringConfigRequest>>
	{
		private DataSourceStringConfigResponse _response;
		private bool _canExecute;

		public DataSourceStringConfigExecutor(GenericInterAppMessage<DataSourceStringConfigRequest> message) : base(message)
		{
			_response = new DataSourceStringConfigResponse
			{
				Success = false,
		...
		public override bool Validate()
		{
			...
			if (String.IsNullOrEmpty(Message.Data.Config))
			{
				_response.Description = "The provided config value is null or empty.";
				return false;
			}

			if (Message.Data.Config == "N/A")
			{
				_response.Description = "The provided config value cannot be 'N/A'.";
				return false;
			}
		...
		private void DataSets_Initial(SLProtocol protocol)
		{
			...
			catch (Exception ex)
			{
				...
				protocol.Log($"QA{protocol.QActionID}|{nameof(DataSourceStringConfigExecutor)}.{nameof(DataSets)}|{Message.Guid}|An error occurred while trying to set the parameter value. Exception details: {ex}", LogType.Error, LogLevel.NoLogging);
			}
		}

		private void DataSets_Next(SLProtocol protocol)
		{
			protocol.SetParameter(Parameter.generic_datasource_postvalidation_configstring_5002, Message.Data.Config);
		...
		public override Message CreateReturnMessage()
		{
			if (_response != null)
			{
				return new GenericInterAppMessage<DataSourceStringConfigResponse>(_response);
			}
